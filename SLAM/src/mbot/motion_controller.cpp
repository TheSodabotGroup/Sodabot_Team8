...
public:
    ...
    */
    MotionController(lcm::LCM * instance) : lcmInstance(instance)
    {
        ...
        /* Roger's parameters initialization */
    ...
    */
    mbot_motor_command_t updateCommand(void)
    {
        ...
        /* Original code */
        const float kPGain = 1.5f;//1.8f;
        const float kDGain = 0.4f;//0.5f;
        const float kIGain = 0.0f;//0.0001f;
        
        const float kSlowdownDist = 0.35f;//0.2f;
        const float kDesiredSpeed = 0.15f;//0.3f;
        ...
        if(!targets_.empty() && !odomTrace_.empty())
        {
            ...
            float error = angle_diff(targetHeading, pose.theta);
            ...
            if(state_ == TURN)
            {
                ...
                /* Original Code */
                if(std::abs(error) > 0.05) //0.10) turn in place until pointed approximately at the target
                {
                ...
                if(distToGoal < kSlowdownDist)
                {
                    ...
                    speed = kMinSpeed + 0.2 * std::sqrt(distToGoal);
                }
...
private:
    ...
    bool haveReachedTarget(void)
    {
        ...
        if(odomTrace_.empty()) return false;
