...
, haveMap_(false)
, haveMap3D_(false) //@Wei
, numIgnoredScans_(0)
, filter_(numParticles)
, map_(20.0f, 20.0f, 0.1f) //30,30,0.1  // create a 10m x 10m grid with 0.05m cells
, map3D_(20.0f, 20.0f, 20.0f, 0.1f)  //  @Wei 3D occupancy grid
, mapper_(5.0f, hitOddsIncrease, missOddsDecrease)
, mapper3D_(5.0f, hitOddsIncrease, missOddsDecrease) //@Wei 3D mapping function
, lcm_(lcmComm)
, mapUpdateCount_(0)
, mapUpdateCount3D_(0) //@Wei 
{
    ...
    if(mode_ == mapping_only)
    {
        ...
        //@Wei, no need to modify
    }
    
    // Zero-out all the poses to start. Either the robot will start at (0,0,0) or at the first pose received from the
    // Optitrack system.
    initialPose_.x = initialPose_.y = initialPose_.theta = initialPose_.tb_angles[0] = 0.0f;
    previousPose_.x = previousPose_.y = previousPose_.theta = previousPose_.tb_angles[0] = 0.0f;
    currentPose_.x  = currentPose_.y  = currentPose_.theta  = currentPose_.tb_angles[0] = 0.0f;
}
...
void OccupancyGridSLAM::handleOdometry(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const odometry_t* odometry)
{
    ...
    odomPose.tb_angles[0] = odometry->tb_angles[0];
    odomPose.tb_angles[1] = odometry->tb_angles[1];
    odomPose.tb_angles[2] = odometry->tb_angles[2];
...
bool OccupancyGridSLAM::isReadyToUpdate(void)
{
    ...
    // std::cout<<"Do I have laser scans ? "<<!incomingScans_.empty()<<"\n";
...
void OccupancyGridSLAM::runSLAMIteration(void)
{
    ...
    if(currentScan_.num_ranges > 100)//250)
    {
        ...
        if (use3D_){ 
            // std::cout<<"Updating 3D Map !\n";
            updateMap3D();
            // std::cout<<"3D Map Updated !\n";
        }
...
//@Wei, currently 3D grid can only works for mapping only, not ready for particle filters, TODO TODO TODO
...
//@Wei added updateMap3D, TODO TODO TODO
void OccupancyGridSLAM::updateMap3D(void)
{
    if(mode_ != localization_only)
    {
        // Process the map
        mapper3D_.updateMap(currentScan_, currentPose_, map3D_);
        haveMap3D_ = true;
    }

    // Publish the map even in localization-only mode to ensure the visualization is meaningful
    // Send every 5th map -- about 1Hz update rate for map output -- can change if want more or less during operation
    if(mapUpdateCount3D_ % 5 == 0)
    {
        auto mapMessage = map3D_.toLCM();
        lcm_.publish(SLAM_MAP_CHANNEL_3D, &mapMessage);
        //map3D_.saveToFile("current.map");
