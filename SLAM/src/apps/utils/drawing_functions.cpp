...
#include <slam/occupancy_grid_3D.hpp> // @Wei: include header new
...
void draw_occupancy_grid_3D(const OccupancyGrid3D& grid, vx_buffer_t* buffer)
{
    ////////////////// TODO: Draw OccupancyGrid as specified in assignment ////////////////////////////
    for(int z = 0; z < grid.depthInCells();z++){ // accessor funcion
    
    //for(int z = 1; z <2;z++){ // accessor funcion
        image_u32_t* img = image_u32_create(grid[z].widthInCells(), grid[z].heightInCells());
        ...
        for(int y = 0; y < grid.heightInCells(); ++y)
        {
            for(int x = 0; x < grid.widthInCells(); ++x)
            {
                ...
                if(grid(x, y, z) > 0) // free space
                {
                    if(z == 0){img->buf[index] = 0xFF0000AA;}
                    else if(z == 1){img->buf[index] = 0xFF0000BB;}
                    else if(z == 2){img->buf[index] = 0xFF0000CC;}
                    else if(z == 4){img->buf[index] = 0xFF0000DD;}
                    else if(z == 5){img->buf[index] = 0xFF0000EE;}
                    else if(z == 6){img->buf[index] = 0xFF0000FF;}
                    else if(z == 7){img->buf[index] = 0xFF00AAFF;}
                    else if(z == 8){img->buf[index] = 0xFF00BBFF;}
                    else if(z == 9){img->buf[index] = 0xFF00CCFF;}
                    else {img->buf[index] = 0xFF00DDFF;}
                    
                }
                else
                {
                    img->buf[index] = 0x00000000;
                }
            }
        }
        ...
        vx_object_t* gridObject = vxo_image_from_u32(img, 0, VX_TEX_MIN_FILTER);
        vx_buffer_add_back(buffer, 
                           vxo_chain(vxo_mat_translate3(grid[z].originInGlobalFrame().x,
                                                        grid[z].originInGlobalFrame().y,
                                                        0.05*z),
                                     vxo_mat_scale(grid.metersPerCell()),
                           gridObject));
