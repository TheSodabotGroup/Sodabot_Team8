...
void BotGui::onDisplayStart(vx_display_t* display)
{
    ...
    lcmInstance_->subscribe(SLAM_MAP_CHANNEL_3D, &BotGui::handleOccupancyGrid_3D, this); //@Wei
}
...
void BotGui::render(void)
{
    ...
    if(haveGrid3D_ && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(showMapCheck_)))
        {draw_occupancy_grid_3D(map3D_, mapBuf);}

    else{draw_occupancy_grid(map_, mapBuf);}
...
//TODO: FIX 3D OCCUPANCY GRID BUFFER INTO THE BOTGUI
//==============================================================================

void BotGui::handleOccupancyGrid_3D(const lcm::ReceiveBuffer* rbuf, 
                                 const std::string& channel, 
                                 const occupancy_grid_3D_t* map)
{
    std::lock_guard<std::mutex> autoLock(vxLock_);
    map3D_.fromLCM(*map);
    haveGrid3D_ = true;
