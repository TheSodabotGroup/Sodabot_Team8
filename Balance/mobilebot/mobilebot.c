...
int main(){
	...
	// TODO: orientation changed TEST IMU direction
    imu_config.orient = ORIENTATION_X_DOWN;
    ...
    mb_state.left_encoder_total = 0;
    mb_state.right_encoder_total = 0;
...
void mobilebot_initialize(){

    pid_params.gain1 = 1.3;
    pid_params.kp1 = -5.07; //-5.15 //4.07
    pid_params.ki1 = -12.42; //10.42
    pid_params.kd1 = -0.2; //-0.209 // 0.081
    // pid_params.tf1 = 0.04;
    pid_params.tf1 = 0.01;
    
    pid_params.gain2 = 1.0;
    pid_params.kp2 = -0.0040; //-0.0204  //0.03
    pid_params.ki2 = -0.0040; //-0.0208 //0.001
    pid_params.kd2 = -0.0100; //-0.0106 //0.004
    // pid_params.tf2 = 0.04;// 0.0002
    pid_params.tf2 = 0.1;
...
void publish_mb_msgs(){
    ...
    odo_msg.tb_angles[0] = mb_state.tb_angles[0];
    odo_msg.tb_angles[1] = mb_state.tb_angles[1];
    odo_msg.tb_angles[2] = mb_state.tb_angles[2];
...
void mobilebot_controller(){
    ...
    mb_controller_update(&mb_state, &mb_setpoints, &mb_odometry, &pid_params);
    if(count % 5 == 0){
        publish_mb_msgs();
    }
    count ++;
		...
		if (rc_dsm_is_new_data()) {
            ...
            if(rc_dsm_ch_normalized(7) > 0.6){
                if(rc_dsm_ch_normalized(5) > 0.6){
                    pid_params.kp1 += 0.1 * rc_dsm_ch_normalized(3);
                }else if(rc_dsm_ch_normalized(5) < 0.4){
                    pid_params.kd1 += 0.01 * rc_dsm_ch_normalized(3);
                }
            }else if(rc_dsm_ch_normalized(7) < 0.4){
                if(rc_dsm_ch_normalized(5) > 0.6){
                    pid_params.kp2 += 0.01 * rc_dsm_ch_normalized(3);
                }else if(rc_dsm_ch_normalized(5) < 0.4){
                    pid_params.kd2 += 0.01 * rc_dsm_ch_normalized(3);
                }
                mb_state.left_encoder_total = 0;
                mb_state.right_encoder_total = 0;
            }
...
void* printf_loop(void* ptr){
	...
	while(rc_get_state()!=EXITING){
		...
		if(new_state == RUNNING){
			...
			printf("%7.3f  |", mb_state.tb_angles[0]);
            ...
            printf("%7.3f  |", mb_state.Kp_err);
            printf("%7.3f  |", mb_state.Ki_err);
            printf("%7.3f  |", mb_state.Kd_err);


            printf("%7.3f  |", mb_state.phi);
            printf("%7.3f  |", mb_state.theta_ref);

            printf("%" PRId64 "  |", mb_state.left_encoder_total);
            printf("%" PRId64 "  |", mb_state.right_encoder_total);
            printf("%7.3f  |", pid_params.kp1);
            printf("%7.3f  |", pid_params.kd1);
            printf("%7.3f  |", pid_params.kp2);
            printf("%7.3f  |", pid_params.kd2);
